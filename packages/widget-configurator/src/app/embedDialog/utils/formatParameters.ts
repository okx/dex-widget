...
export function formatParameters(params: any, padLeft = 0, isTypescript: boolean): string {
    // Stringify params
    ...
    const resultWithComments = Object.keys(commentsByParamName).reduce((acc, propName) => {
        return acc.replace(
            ...
            `$& // ${commentsByParamName[propName]}`,
        );
    }, formattedParams);

    // Add values
    const tradeTypeValue = isTypescript
    ...
    let resultWithValues = Object.keys(valuesByParamName).reduce((acc, propName) => {
        return acc.replace(
            new RegExp(`("${propName}".* )(".*")(.*)$`, 'gm'),
            `$1${valuesByParamName[propName]}$3`,
        );
    }, resultWithComments);
    ...
    if (isTypescript) {
        resultWithValues = resultWithValues.replace(
            new RegExp(/^(\s*)"(\w*)"(,?)$/gm),
            (_match, space, tradeType, comma) =>
                space + 'TradeType.' + tradeType.toUpperCase() + comma,
        );
    }
